...
desc inventory;
alter table inventory modify column  quantity int (10) default '0';
...
alter table products modify column product_id int(10) auto_increment ,
auto_increment =1001;
...
alter table locations add column region_id int(10);

-- 3. Write a SQL statement to add a column ID as the first column of the table locations.

desc locations;
alter table locations add column column_id int(10) first;
...
alter table locations add column country_id  int(10);
...
change  column STATE_PROVINCE  state varchar(25);

-- 8. Write a SQL statement to add a primary key for the columns

alter table locations add primary key (region_id);
...
-- 13 13. Write a SQL statement to drop the existing foreign key fk_job_id from the job_history table on job_id column which is referencing the job_id of jobs table.
...
alter table job_history change column job_id  indx_job_id varchar(100); 
alter table job_history  add index (indx_job_id);
...
alter table employees add column statuss varchar(10) default 'active';
select * from employees; 
...
alter table departments modify column department_name varchar(20) not null;
select * from departments;
...
select * from employees;
desc employees;
alter table employees add column statuss varchar(10) default 'inactive';
...
desc employees;
alter table employees modify column salary int after last_name;
-- 26.Write a query to remove the default value from the status column in the employees table.

alter table employees modify column statuss  varchar(10);
...
alter table users modify column uc_email varchar(30),
  add constraint  unique_email_constrint unique (uc_email);
...
select * from employees;
alter table employees add column age smallint null;
